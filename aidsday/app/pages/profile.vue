...
// 默认值设置为 -1，表示未选择
let gender = ref<number>(-1); // 1: 小哥哥, 2: 小姐姐
let age = ref<number>(-1); // 1: 0-17岁, 2: 18-25岁, 3: 26-40岁, 4: 41-60岁, 5: 61岁及以上
  ...
  <div class="min-h-screen flex flex-col items-center bg-cover bg-center bg-[#51BDDA] relative">
    ...
    <div class="absolute top-0 left-0 w-full px-6">
      ...
      <div class="flex gap-4 w-full max-w-xl pt-[1.6rem] mb-[1rem]">
        ...
        <div @click="setGender(2)" class="flex-1 cursor-pointer">
          <div class="flex flex-col items-center">
            <div
                class="w-full h-36 border-2 rounded-xl mb-4 flex items-center justify-center overflow-hidden transition duration-200"
                :class="{
                'border-[#FFCA3C]': gender === 2, // 选中时边框色
                'border-white': gender !== 2,     // 未选中时边框色
              }"
            >
            ...
            <div
                class="mt-2 py-1 px-6 rounded-full font-bold text-base shadow-md transition duration-200"
                :class="{
                'bg-yellow-400 text-white border border-white': gender === 2,     // 选中时样式
                'bg-white text-[#5AD3E7] border border-gray-300': gender !== 2, // 未选中时样式
              }"
            >
              小姐姐
            </div>
      ...
      <div class="grid grid-cols-3 gap-4 pt-[1.6rem] w-full max-w-xl">
        <div
            ...
            :class="{
            'bg-yellow-400 text-white border border-white font-bold': age === 1,        // 选中样式
            'text-[#5AD3E7] border border-gray-300 bg-white': age !== 1, // 未选中样式
          }"
        >
          0-17岁
        </div>
        ...
        <div
            ...
            class="w-[5rem] h-[1.8rem] text-[0.7rem] flex items-center justify-center rounded-full cursor-pointer font-normal transition duration-150 ease-in-out"
            :class="{
            'bg-yellow-400 text-white border border-white font-bold': age === 2,        // 选中样式
            'text-[#5AD3E7] border border-gray-300 bg-white': age !== 2, // 未选中样式
          }"
        >
          18-25岁
        </div>
        ...
        <div
            ...
            class="w-[5rem] h-[1.8rem] text-[0.7rem] flex items-center justify-center rounded-full cursor-pointer font-normal transition duration-150 ease-in-out"
            :class="{
            'bg-yellow-400 text-white border border-white font-bold': age === 3,        // 选中样式
            'text-[#5AD3E7] border border-gray-300 bg-white': age !== 3, // 未选中样式
          }"
        >
          26-40岁
        </div>
        ...
        <div
            ...
            class="w-[5rem] h-[1.8rem] text-[0.7rem] flex items-center justify-center rounded-full cursor-pointer font-normal transition duration-150 ease-in-out"
            :class="{
            'bg-yellow-400 text-white border border-white font-bold': age === 4,        // 选中样式
            'text-[#5AD3E7] border border-gray-300 bg-white': age !== 4, // 未选中样式
          }"
        >
          41-60岁
        </div>
        ...
        <div
            ...
            class="w-[5rem] h-[1.8rem] text-[0.7rem] flex items-center justify-center rounded-full cursor-pointer font-normal transition duration-150 ease-in-out"
            :class="{
            'bg-yellow-400 text-white border border-white font-bold': age === 5,        // 选中样式
            'text-[#5AD3E7] border border-gray-300 bg-white': age !== 5, // 未选中样式
          }"
        >
          61岁及以上
        </div>
